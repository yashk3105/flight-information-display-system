...
class ArrivalPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label3 = tk.Label(self, text="Airline: ")
...
class ArrivalUpdate(tk.Frame):
    ...
    def on_button(self):
        ...
        conn.execute('''UPDATE ArrivalFlight SET Remarks = ? WHERE Flight_Name = ?''', (remarks, flightname))
        conn.commit()
...
class DepatureUpdate(tk.Frame):
    ...
    def on_button(self):
        ...
        c.execute('SELECT * FROM DepatureFlight')
        conn.execute('''UPDATE DepatureFlight SET Remarks = ? WHERE Flight_Name = ?''', (remarks, flightname))
        conn.commit()
